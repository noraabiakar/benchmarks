...
inline void flop_0(int64_t size, double* data_a) // 2 flops - 8 bytes = 1/4 flops/byte 
{
    ...
    for (unsigned i = 0; i < size; i+=8) {
       ...
       auto t0 = svdiv_f64_z(pg, three, svadd_f64_z(pg, a, two));
...
inline void flop_1(int64_t size, double* data_a) // 5 flops - 8 bytes = 5/8 flops/byte 
{
    ...
    for (unsigned i = 0; i < size; i+=8) {
       ...
       auto t0 = svdiv_f64_z(pg, three, svadd_f64_z(pg, a, two));
       auto t1 = svdiv_f64_z(pg, seven, svadd_f64_z(pg, a, one));
...
inline void flop_2(int64_t size, double* data_a) // 11 flops - 8 bytes = 11/8 flops/byte 
{
    ...
    for (unsigned i = 0; i < size; i+=8) {
       ...
       auto t0 = svdiv_f64_z(pg, three,svadd_f64_z(pg, a, two));
       auto t1 = svdiv_f64_z(pg, seven,svadd_f64_z(pg, a, one));
       auto t2 = svdiv_f64_z(pg, two,  svadd_f64_z(pg, a, three));
       auto t3 = svdiv_f64_z(pg, one,  svadd_f64_z(pg, a, seven));
...
inline void flop_3(int64_t size, double* data_a) // 17 flops - 8 bytes = 2 flops/byte 
{
    ...
    for (unsigned i = 0; i < size; i+=8) {
       ...
       auto t2 = svdiv_f64_z(pg, two,  svadd_f64_z(pg,a, three));
       auto t3 = svdiv_f64_z(pg, one,  svadd_f64_z(pg,a, seven));
       auto t4 = svdiv_f64_z(pg, one,svadd_f64_z(pg, a, three));
       auto t5 = svdiv_f64_z(pg, two,svadd_f64_z(pg, a, seven));
...
inline void flop_4(int64_t size, double* data_a) // 35 flops - 8 bytes = 4.4 flops/byte 
{
    ...
    for (unsigned i = 0; i < size; i+=8) {
       ...
       auto t4 = svdiv_f64_z(pg, one,  svadd_f64_z(pg, a, three));
       auto t5 = svdiv_f64_z(pg, two,  svadd_f64_z(pg, a, seven));
       auto t6 = svdiv_f64_z(pg, one,  svadd_f64_z(pg, a, two));
       auto t7 = svdiv_f64_z(pg, three,svadd_f64_z(pg, a, seven));
       auto t8 = svdiv_f64_z(pg, three,svadd_f64_z(pg, a, one));

       t0 = svdiv_f64_z(pg, t3,svadd_f64_z(pg, t4, t0)); 
       t1 = svdiv_f64_z(pg, t5,svadd_f64_z(pg, t6, t1)); 
       t2 = svdiv_f64_z(pg, t7,svadd_f64_z(pg, t8, t2)); 
       t3 = svdiv_f64_z(pg, t5,svadd_f64_z(pg, t4, t3)); 
       t4 = svdiv_f64_z(pg, t7,svadd_f64_z(pg, t6, t4)); 
       t5 = svdiv_f64_z(pg, t8,svadd_f64_z(pg, t6, t5)); 
...
inline void flop_5(int64_t size, double* data_a) // 62 flops - 8 bytes = 7.75 flops/byte 
{
    ...
    for (unsigned i = 0; i < size; i+=8) {
       ...
       t4 = svdiv_f64_z(pg, t7,   svadd_f64_z(pg, t6, t4)); 
       t5 = svdiv_f64_z(pg, t8,   svadd_f64_z(pg, t6, t5)); 
       t6 = svdiv_f64_z(pg, three,svadd_f64_z(pg, t7, t6)); 
       t7 = svdiv_f64_z(pg, one,  svadd_f64_z(pg, t8, t7)); 
       t8 = svdiv_f64_z(pg, two,  svadd_f64_z(pg, t8, t8)); 
